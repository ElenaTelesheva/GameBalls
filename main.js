...
class Hexagon {
    ...
    paintRandomRound() {
        ...
        this.arr[3][3].color = "red"

    }

    paintCenterRound() {
        //this.arr[3][7].color = "red"
        //this.arr[2][4].color = "red"
        this.arr[4][4].color = "red"
        ...
        for (let j = 0; j < x.m; j++) {
            if (x.arr[i][j] != null) {
                if (indexes.indexOf(x.arr[i][j].number) != -1) {
                    ...
                    if (!O.find(i => JSON.stringify(i) === JSON.stringify(newChildren)) && !isInSet(newChildren.arr)) {
                        O.push(newChildren);
                    }
        ...
        for (let j = 0; j < x.m; j++) {
            if (x.arr[i][j] != null) {
                if (indexes.indexOf(x.arr[i][j].number) != -1) {
                    ...
                    newChildren.arr[i][j - 2].color = mass[length - counter];
                    newChildren.arr[i - 1][j - 1].color = mass[length - counter]; counter++;
                    newChildren.arr[i - 1][j + 1].color = mass[length - counter]; counter++;
                    newChildren.arr[i][j + 2].color = mass[length - counter]; counter++;
                    newChildren.arr[i + 1][j + 1].color = mass[length - counter]; counter++;
                    newChildren.arr[i + 1][j - 1].color = mass[length - counter]; counter++;
...
let isInSet = (array2) => {
    ...
    C.forEach(function(value, valueAgain, set){
        ...
        if (!isIqualMass(value.arr, array2, value.n, value.m )) is = 0;
    })
    console.log(is);
    return is;
}
...
let play = () => {
    ...
    while (O.length != 0) {
        ...
        if (isIqualMass(x.arr, aim.arr, x.n, x.m)) {
            //x.drawField();//отрисовали результат
